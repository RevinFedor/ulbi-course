...
export const ArticleEditBlockHeader = memo((props: ArticleEditBlockHeaderProps) => {
  ...
  }, [articleForm, updateArticleQuery, addArticleQuery, isLoadingUpdate, id, navigate]);

  return (
    <div className="mb-4 flex flex-col f-full justify-center">
      <Text title={`Редактирование статьи и id - ${id}`} />
      <div className="flex justify-between gap-4">
        <button className={`${cls.Links}  text-[#ff0404] border-[#ff0404]`} onClick={onCancelEdit}>
          {t('Отменить')}
        </button>
        <button
        ...
        <button className={`${cls.Links} text-[#04ff04] border-[#04ff04]`} onClick={onSaveEdit}>
          Сохранить
        </button>
      </div>
    </div>
